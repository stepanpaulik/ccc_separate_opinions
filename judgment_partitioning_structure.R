...
data = readr::read_rds("../data/judgments_annotated_doc2vec.rds")
# write_rds(data, "../data/judgments_annotated_doc2vec.rds")
# data = readr::read_rds("../data/judgments_annotated_dtm.rds")
metadata = readr::read_rds("../data/US_metadata.rds")
...
model_structure = wflow_set_tune %>%
  extract_workflow("recipe_xgboost") %>%
  finalize_workflow(best_results) %>%
  fit(data_structure)
